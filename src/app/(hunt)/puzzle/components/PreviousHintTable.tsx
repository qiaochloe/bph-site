...
export default function PreviousHintTable({
  ...
  const handleSubmitEdit = async (id: number, value: string, type: MessageType) => {
    switch (type) {
      case "request":
        // Optimistic update
        startTransition(() => {
  ...
  const handleSubmitFollowUp = async (hintId: number, message: string) => {
    ...
    startTransition(() => {
      setOptimisticHints((prev) =>
        prev.map((hint) =>
          hint.id === hintId ?
            ({ ...hint, followUps: hint.followUps.concat({ id: 0, message, canEdit: false }) })
            : hint))
    }
    )
    ...
    if (!followUpId) {
      ...
      startTransition(() => {
        setOptimisticHints((prev) =>
          prev.map((hint) =>
            hint.id === hintId ?
              ({ ...hint, followUps: hint.followUps.filter((followUp) => followUp.id !== 0) })
              : hint))
      });
    } else {
      // Update followUpId
          ...
          prev.map((hint) =>
            hint.id === hintId
              ? {
                ...hint,
                followUps: hint.followUps.map((followUp) =>
                  followUp.id === 0 ? { ...followUp, id: followUpId, canEdit: true } : followUp
                ),
              }
              : hint
          )
  ...
  const handleFollowUpChange = (hintId: number, e: React.ChangeEvent<HTMLTextAreaElement>) => {
          ...
          <>
            ...
            <TableRow key={`${hint.id}-request`}>
              <TableCell className="break-words">
                <div className="pb-4 flex justify-between">
                  ...
                  <div className="p-1">
                    {edit?.id !== hint.id ?
                      <button onClick={() => handleEdit(hint.id, hint.request, "request")} className="underline">Edit</button> :
                      <div className="space-x-2">
                        <button onClick={() => handleSubmitEdit(edit.id, edit.value, "request")} className="underline">Save</button>
                        <button onClick={() => setEdit(null)} className="underline">Cancel</button>
                      </div>
                    }
                  </div>
                </div>
                ...
                <TableCell className="break-words" onClick={() => handleHideFollowUps(hint.id)}>
                  <div className="flex justify-between pb-4">
                    ...
                    {hint.response &&
                      <div className="p-1">
                        {followUp === null ?
                          <button onClick={() => setFollowUp({ hintId: hint.id, message: "" })} className="underline">Follow-Up</button> :
                          <button onClick={() => setFollowUp(null)} className="underline">Cancel</button>
                        }
                      </div>
                    }
                  </div>
                  ...
                  <div className="pb-4 flex justify-between">
                    ...
                    <div className="p-1">
                      {edit?.type === "follow-up" && edit.id === followUp.id ?
                        <button onClick={() => handleSubmitEdit(followUp.id, edit.value, "follow-up")} className="underline">Save</button> :
                        <button onClick={() => handleEdit(followUp.id, followUp.message, "follow-up")} className="underline">Edit</button>
                      }
                    </div>
                  </div>
                  <div className="pb-4 p-1">
            ...
            {followUp !== null && followUp.hintId === hint.id &&
              <TableRow key={`${hint.id}-follow-up-request`}>
                <TableCell className="bg-gray-200 break-words">
                  <div className="pb-4">
                    <p className="p-1">
                      Follow-Up
                    </p>
                  ...
                  <div className="flex space-x-2 p-1">
                    <button onClick={() => handleSubmitFollowUp(hint.id, followUp.message)} className="underline">Submit</button>
                    <button onClick={() => setFollowUp(null)} className="underline">Cancel</button>
                  </div>
                </TableCell>
              </TableRow>
