...
export default auth(async (req) => {
  ...
  //   // After the hunt ends, all puzzles can be accessed
  //   if (new Date() > HUNT_END_TIME) {
  //     return;
  //   }
  //
  //   // Unauthenticated users can't access puzzles
  //   if (!req.auth?.user?.id) {
  //     const newUrl = new URL("/login", req.nextUrl.origin);
  //     return Response.redirect(newUrl);
  //   }
  //
  //   // Authenticated users can't access puzzles before the hunt starts
